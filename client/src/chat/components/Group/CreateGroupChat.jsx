...
function GroupChatCreate({ setModalActive }) {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const getUsers = async () => {
        const { data } = await axios.get(`${allUsersRoute}?search=${query}`)
        setLoading(false)
  ...
  return (
    <>
      <div style={{zIndex:'200'}} className="create-group modal-wrapper">
        <div className={`modal-container ${activeSearch ? 'active-search' : ''}`}>
          ...
          <form className="modal-content" onSubmit={(event) => handleSubmit(event)}>
            <Stack>
              <TextInput
                rightSection={<BsPencil />}
                type="text"
                placeholder="Chat Name"
                onChange={(e) => setGroupChatName(e.target.value)}
              />
              <TextInput
                rightSection={<AiOutlineUserAdd />}
                type="text"
            ...
            <Button m={'md'} type="submit">Create Chat</Button>
          </form>
        </div>
      </div>
      {/* <ToastContainer /> */}
